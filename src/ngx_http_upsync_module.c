...
#define NGX_HTTP_UPSYNC_ETCD_V3              0x0004
...
static ngx_int_t ngx_http_upsync_add_peers(ngx_cycle_t *cycle, 
    ngx_http_upsync_server_t *upsync_server);
static void ngx_http_upsync_add_filter(ngx_cycle_t *cycle, 
    ngx_http_upsync_server_t *upsync_server);
static ngx_int_t ngx_http_upsync_del_peers(ngx_cycle_t *cycle,
    ngx_http_upsync_server_t *upsync_server);
static void ngx_http_upsync_del_filter(ngx_cycle_t *cycle, 
    ngx_http_upsync_server_t *upsync_server);

static ngx_int_t ngx_http_upsync_update_peers(ngx_cycle_t *cycle,
    ngx_http_upsync_server_t *upsync_server);
...
static void etd3_watch_body(u_char *body, ngx_str_t *key, ngx_str_t *range_end);
...
ngx_http_upsync_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "etcd3_key=", 10) == 0) {
            ...
            if (upscf->upsync_etcd3_key.len == 0) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
...
{
    ngx_uint_t                   add_flag = 0, del_flag = 0, update_flag = 0;
    ngx_upsync_conf_t           *upsync_type_conf;
    ngx_http_upsync_ctx_t       *ctx;
    ...
    ngx_http_upsync_del_filter((ngx_cycle_t *)ngx_cycle, upsync_server);
    ...
    if (!add_flag && !del_flag) {
        if (ngx_http_upsync_update_peers((ngx_cycle_t *)ngx_cycle, 
                                           upsync_server) != NGX_OK) {
            ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
    ...
    if (add_flag || del_flag || update_flag) {
...
ngx_http_upsync_check_index(ngx_http_upsync_server_t *upsync_server)
{
    ...
    if (upsync_type_conf->upsync_type == NGX_HTTP_UPSYNC_ETCD_V3) {
        cJSON *root = cJSON_Parse((char *)&state.http_body);
        if (root == NULL) {
            ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                          "upsync_check_index: parse etcd3 revision error");
            return NGX_ERROR;
        }
        ...
        if (result != NULL) {
            header = cJSON_GetObjectItem(result, "header");
        } else {
        ...
        if (header != NULL && header->valuestring != NULL) {
            ...
            if (revision != NULL && revision->valuestring != NULL) {
                index = ngx_strtoull(revision->valuestring, (char **) NULL, 10);
            }
        }
...
static void
ngx_http_upsync_add_filter(ngx_cycle_t *cycle, 
    ngx_http_upsync_server_t *upsync_server)
{
    ngx_uint_t                          i, j, len;
    ngx_http_upsync_ctx_t              *ctx;
...
static void
ngx_http_upsync_del_filter(ngx_cycle_t *cycle, 
    ngx_http_upsync_server_t *upsync_server)
{
    ngx_uint_t                         i, len;
    ngx_http_upsync_ctx_t             *ctx;
...
static ngx_int_t
ngx_http_upsync_update_peers(ngx_cycle_t *cycle,
    ngx_http_upsync_server_t *upsync_server)
{
    ngx_uint_t                           i, w=0, len=0;
    ngx_http_upsync_ctx_t               *ctx;
    ngx_http_upsync_conf_t              *upstream_conf;
    ngx_http_upstream_srv_conf_t        *uscf;
    ngx_http_upstream_rr_peer_t         *peer = NULL;
...
ngx_http_upsync_etcd3_parse_json(void *data)
{
    ...
    {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                      "upsync_etcd3_parse_json: init arrays error");
        cJSON_Delete(root);
        return NGX_ERROR;
    }
    ...
    if (result != NULL) {
        ...
        {
            kv = cJSON_GetObjectItem(event_next, "kv");
            if (kv == NULL) {
                ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
            ...
            if (temp0 != NULL && temp0->valuestring != NULL) {
                ...
                pkey = (u_char *)ngx_strrchr((char *)key.data, '/');
            }

            temp0 = cJSON_GetObjectItem(event_next, "type");
            if (temp0 != NULL && ngx_memcmp(temp0->valuestring, "DELETE", 6) == 0) {
                upstream_conf = ngx_array_push(&ctx->del_upstream);
                ngx_memzero(upstream_conf, sizeof(*upstream_conf));
                ngx_sprintf(upstream_conf->sockaddr, "%*s", ngx_strlen(pkey + 1), pkey + 1);
                continue;
            }

            temp0 = cJSON_GetObjectItem(event_next, "prev_kv");
            if (temp0 == NULL) {
                upstream_conf = ngx_array_push(&ctx->add_upstream);
                ngx_memzero(upstream_conf, sizeof(*upstream_conf));
                ngx_sprintf(upstream_conf->sockaddr, "%*s", ngx_strlen(pkey + 1), pkey + 1);
            } else {
                upstream_conf = ngx_array_push(&ctx->upstream_conf);
            ...
            if (temp0 != NULL && ngx_strlen(temp0->valuestring) != 0) {
                if (ngx_http_upsync_etcd3_parse_json_kv_value(
                        ctx->pool, temp0, upstream_conf) != NGX_OK)
                {
                    ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                                  "upsync_etcd3_parse_json: parse value failed");
                    continue;
                }
            }
            temp0 = NULL;
        ...
        ngx_uint_t add_err = 0, del_err = 0, update_err = 0;
        if (ctx->add_upstream.nelts > 0) {
            if (ngx_http_upsync_add_peers((ngx_cycle_t *)ngx_cycle, upsync_server) != NGX_OK) {
                ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                              "upsync_etcd3_parse_json: upstream add peers failed");
                add_err = 1;
            }
        }

        if (ctx->del_upstream.nelts > 0) {
            if (ngx_http_upsync_del_peers((ngx_cycle_t *)ngx_cycle, upsync_server) != NGX_OK) {
                ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                              "upsync_etcd3_parse_json: upstream del peers failed");
                del_err = 1;
            }
        }

        if (ngx_http_upsync_update_peers((ngx_cycle_t *)ngx_cycle, upsync_server) != NGX_OK) {
            ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                          "upsync_etcd3_parse_json: upstream update peer error");
            update_err = 1;
        }

        cJSON_Delete(root);

        if (add_err || del_err || update_err ) {
            return NGX_ERROR;
        }
        goto dumpfile;
    }

    // response from kvrange
    ...
    cJSON *server_next, *temp0;
    for (server_next = kvs->child; server_next != NULL;
         server_next = server_next->next)
    {
        temp0 = cJSON_GetObjectItem(server_next, "key");
        if (temp0 != NULL && temp0->valuestring != NULL) {
            base64_src.len = ngx_strlen(temp0->valuestring);
            base64_src.data = (u_char *) temp0->valuestring;
            if (etcd3_decode_base64(ctx->pool, &key, &base64_src) != NGX_OK) {
...
dumpfile:
    ...
    // return NGX_ERROR back to ngx_http_upsync_process
    // make ngx_http_upsync_process return earlier
    return NGX_ERROR;
}
...
ngx_http_upsync_etcd3_parse_json_kv_value(ngx_pool_t *pool, cJSON *root, ngx_http_upsync_conf_t *upstream_conf)
{
    ...
    ngx_int_t                      max_fails=1, backup=0, down=0;


    /* default value, server attribute */
    upstream_conf->weight = 1;
    upstream_conf->max_fails = 1;
    upstream_conf->fail_timeout = 10;
    upstream_conf->down = 0;
    upstream_conf->backup = 0;
    ...
    if (etcd3_decode_base64(pool, &base64_dst, &base64_src) != NGX_OK) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
    ...
    if (upstream_conf->weight <= 0) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                      "etcd3_parse_json_kv_value: \"weight\" value is invalid"
                              ", setting default value 1");
        upstream_conf->weight = 1;
    }
    ...
    if (max_fails < 0) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                      "etcd3_parse_json_kv_value: \"max_fails\" value is invalid"
                              ", setting default value 1");
    } else {
        upstream_conf->max_fails = (ngx_uint_t)max_fails;
    }
    ...
    if (upstream_conf->fail_timeout < 0) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                      "etcd3_parse_json_kv_value: \"fail_timeout\" value is invalid"
                              ", setting default value 10");
        upstream_conf->fail_timeout = 10;
    }
    ...
    if (down != 1 && down != 0) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                      "etcd3_parse_json_kv_value: \"down\" value is invalid"
                              ", setting default value 0");
    } else {
        upstream_conf->down = (ngx_uint_t)down;
    }
    ...
    if (backup != 1 && backup != 0) {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                      "etcd3_parse_json_kv_value: \"backup\" value is invalid"
                              ", setting default value 0");
    } else {
        upstream_conf->backup = (ngx_uint_t)backup;
    }

    max_fails=1, backup=0, down=0;
...
ngx_http_upsync_send_handler(ngx_event_t *event)
{
    ...
    if (upsync_type_conf->upsync_type == NGX_HTTP_UPSYNC_ETCD_V3) {
        ...
        size_t body_size = key.len + range_end.len  + 64;
...
ngx_http_upsync_clean_event(void *data)
{
    ...
    if (upsync_type_conf->upsync_type == NGX_HTTP_UPSYNC_CONSUL
        ...
        || upsync_type_conf->upsync_type == NGX_HTTP_UPSYNC_ETCD_V3)
    {
        if (parser != NULL) {
            ngx_free(parser);
            parser = NULL;
...
etcd3_get_prefix(u_char *end, ngx_str_t *key)
{
    ...
    // should use noPrefixEnd instead of end
    return NGX_ERROR;
}
...
etcd3_decode_base64(ngx_pool_t *pool, ngx_str_t *dst, ngx_str_t *src) {
    ...
    dst->data = p;
    return ngx_decode_base64(dst, src);
}
 ...
 */
static void
etd3_watch_body(u_char *body, ngx_str_t *key, ngx_str_t *range_end)
{
    ngx_sprintf(body, "{\"create_request\":{\"key\":\"%V\",\"range_end\":\"%V\",\"prev_kv\":true}}",
                key, range_end);
}